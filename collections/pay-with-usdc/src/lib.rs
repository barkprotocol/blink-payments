...
const USDC_MINT_ADDRESS: Pubkey = pubkey!("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v");
...
pub mod pay_with_usdc { 
    ...
    fn pay(ctx: Context<PayAction>) -> Result<ActionTransaction> {
        ...
        let receiver_usdc_ata_address = get_associated_token_address(&receiver_pubkey, &USDC_MINT_ADDRESS);

        let create_usdc_ata_instruction = create_associated_token_account_idempotent(
            &account_pubkey,
            &receiver_pubkey,
            &USDC_MINT_ADDRESS,
            &TOKEN_PROGRAM_ID,
        ...
        let mut instructions = vec![create_usdc_ata_instruction];
